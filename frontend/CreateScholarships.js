// CreateScholarships.js - Simplified to match CCC Events pattern
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function getFormData() {
    ...
    const formData = {
      title: document.getElementById("scholarshipTitle")?.value.trim() || "",
  ...
  function loadDraftData() {
    ...
    if (isEditMode && savedDraft) {
      try {
        ...
        if (data.title)
          document.getElementById("scholarshipTitle").value = data.title;
  ...
  function populateFormFromData(data = {}) {
    if (data.title) document.getElementById("scholarshipTitle").value = data.title;
  ...
  if (typeof setupSubmissionView === "function") {
    const isSubmissionView = setupSubmissionView({
      ...
      formElement: createScholarshipForm,
      extraDisableElements: [draftBtn],
    });

    if (isSubmissionView) {
      console.log("📖 Viewing scholarship submission in read-only mode.");
    }
  }

  // Handle form submission
  if (createScholarshipForm) {
    createScholarshipForm.addEventListener("submit", async function (e) {
      e.preventDefault();
      ...
      try {
        ...
        Toast.success("Scholarship submitted for review!");
  ...
  if (draftBtn) {
    draftBtn.addEventListener("click", async function (e) {
      ...
      if (createScholarshipForm) {
        createScholarshipForm.noValidate = true;
      }
