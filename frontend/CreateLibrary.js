...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadDraftData() {
    ...
    if (isEditMode && savedDraft) {
      try {
        ...
        if (data.title)
          document.getElementById("libraryTitle").value = data.title;
  ...
  function populateFormFromData(data = {}) {
    if (data.title) document.getElementById("libraryTitle").value = data.title;
  ...
  if (typeof setupSubmissionView === "function") {
    const isSubmissionView = setupSubmissionView({
      ...
      formElement: createLibraryForm,
      extraDisableElements: [draftBtn],
    });

    if (isSubmissionView) {
      console.log("📖 Viewing Library submission in read-only mode.");
    }
  }

  // Handle form submission
  if (createLibraryForm) {
    createLibraryForm.addEventListener("submit", async function (e) {
      e.preventDefault();
      ...
      try {
        ...
        Toast.success("Library submitted for review!");
  ...
  if (draftBtn) {
    draftBtn.addEventListener("click", async function (e) {
      ...
      if (createLibraryForm) {
        createLibraryForm.noValidate = true;
      }
