...
async function openModernEditor() {
  try {
    ...
    const publishedItems = items.filter(item => {
      ...
      const status = item.status || item.publishStatus || item.state;
      const isPublished = item.isPublished !== false && item.isPublished !== undefined ? item.isPublished : true;

      // Consider published if status is 'published', 'active', or if no status field exists (backward compatibility)
      return status === 'published' || status === 'active' || (!status && isPublished);
    });

    updateLoadingProgress(10, 'Initializing editor...');
    console.log('Opening editor with', publishedItems.length, 'published items');

    // Use setTimeout to allow UI to render the loading screen
    setTimeout(() => {
      try {
        createModernEditor(publishedItems);
      } catch (error) {
        hideLoadingScreen();
        console.error('Error in createModernEditor:', error);
        if (typeof Toast !== 'undefined') {
          Toast.error('Error creating editor: ' + error.message);
        } else {
...
// Backward-compatible wrapper function for general newsletter editor
// This allows printPage.js and other code to call openGeneralEditor(items)
async function openGeneralEditor(items) {
  console.log('🔄 openGeneralEditor called with', items?.length || 0, 'items');

  // Store items in the global variables that openModernEditor expects
  window.vcNewsletterFilteredItems = items;
  window.vcNewsletterAllItems = items;

  // Call the modern editor
  return openModernEditor();
}
...
async function createModernEditor(publishedItems) {
  try {
    ...
    <div class="modern-header">
      <h2>✨ General Newsletter Editor</h2>
    ...
    <div class="modern-toolbar">
      ...
      <div class="tool-group">
        ...
        <button class="tool-btn" id="cleanParagraphsBtn" title="Clean Paragraphs (Remove gaps & fix line breaks)">
      ...
      <div class="modern-sidebar" id="modernSidebar">
        ...
        <div class="sidebar-section">
          ...
          <div class="template-grid" id="templateGrid">
            ...
            <div class="template-card" data-template="footer">
              <div class="template-icon">📌</div>
              <div class="template-name">Footer Block</div>
            </div>
          </div>
        </div>
      </div>
  ...
  `;
    ...
    // Main title
    const coverTitle = new fabric.IText('GENERAL', {
      left: 397,
      top: 400,
      fontSize: 72,
      fontWeight: 'bold',
    ...
    await addModernContent(contentCanvas, publishedItems);
    ...
    const sidebarPromise = new Promise((resolve) => {
      try {
        populateModernSidebar(publishedItems);
        resolve();
      } catch (error) {
    ...
    // Wait for sidebar or timeout after 2 seconds
    await Promise.race([
      sidebarPromise,
    ...
    // Fallback: force hide loading screen after 5 seconds in case of any issues
    setTimeout(() => {
...
async function addModernContent(canvas, publishedItems) {
  let y = 60;

  console.log(`📊 Processing ${publishedItems.length} published articles`);

  if (publishedItems.length === 0) {
    hideLoadingScreen();
    if (typeof Toast !== 'undefined') {
      Toast.warning('No published articles found. Only published articles can be included in the newsletter PDF.');
    } else {
      alert('No published articles found. Only published articles can be included in the newsletter PDF.');
    }
    return;
  }
  ...
  const title = new fabric.IText('GENERAL NEWSLETTER', {
    left: 397,
    top: 50,
    fontSize: 52,
    fontWeight: 'bold',
  ...
  // Helper function to create text with italic support and balanced typography
  function createTextWithItalics(text, options) {
    // First, balance the text for better typography
    const balancedText = balanceTextContent(text, options.width || 400, options.fontSize || 16, options.fontFamily || 'Arial');

    // Check if text contains <i> tags
    if (!balancedText.includes('<i>') && !balancedText.includes('</i>')) {
      // No italic tags, create simple text
      return new fabric.Textbox(balancedText, options);
    }

    // Parse HTML and create text with styles
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = balancedText;
    ...
    while ((match = italicRegex.exec(balancedText)) !== null) {
      const italicText = match[1];
      const beforeText = balancedText.substring(0, match.index).replace(/<\/?i>/g, '');
  ...
  // Define categories for General Newsletter
  const categories = [
    { id: 'major-events', name: 'Major Events' },
    { id: 'alumni-stories', name: 'Alumni Stories' },
    { id: 'ccc-events', name: 'CCC Events' },
    { id: 'club-activities', name: 'Club Activities' },
    { id: 'degree-review', name: 'Degree Review' },
    { id: 'dept-activities', name: 'Department Activities' },
    ...
    { id: 'training-program', name: 'Training Program' },
  ...
  console.log(`📈 Processing ${totalItems} published articles across ${categories.length} categories`);
  ...
  for (const category of categories) {
    ...
    // New page check for Category Header
    // STRICT: Always start a new page for a new category (unless we are at the very top of a fresh page)
    if (processedItems > 0 && y > 100) {
      addNewPage();
      canvas = window.modernEditor.pages[window.modernEditor.pages.length - 1];
      y = 60;
    }

    // Modern Category header with gradient
    const categoryGradient = new fabric.Gradient({
      type: 'linear',
      coords: { x1: 50, y1: y, x2: 744, y2: y },
      colorStops: [
    ...
    const categoryIcon = new fabric.IText('◆', {
      left: 70,
      top: y + 10,
      fontSize: 20,
      fill: '#ffffff',
    ...
    canvas.add(categoryIcon);

    const categoryHeader = new fabric.IText(category.name.toUpperCase(), {
      left: 100,
      top: y + 12,
      fontSize: 24,
      fontWeight: 'bold',
    ...
    // Add items in this category with alternating layouts
    for (let itemIndex = 0; itemIndex < categoryItems.length; itemIndex++) {
      ...
      // Check if item has images - robust check for multiple property names
      let imageUrls = [];

      const potentialImageProps = ['imageUrls', 'images', 'image', 'featuredImage', 'coverImage', 'photo'];

      for (const prop of potentialImageProps) {
        if (item[prop]) {
          const val = item[prop];
      ...
      const imageOnLeft = hasImages && isEven;

      // Dynamic layout dimensions
      const contentWidth = 674;
      const leftMargin = 60;
      const cardPadding = 15; // Reduced padding for compact layout
      ...
      // Vary accent colors
      const accentColors = ['#667eea', '#764ba2', '#f093fb', '#4facfe', '#43e97b'];
      const accentColor = accentColors[itemIndex % accentColors.length];

      // 1. Prepare Title
      const titleTextStr = getTitleForItem(item);
      const titleObj = createTextWithItalics(titleTextStr, {
        width: contentWidth - (cardPadding * 2), // Fit within card width
        fontSize: 16,
        fontWeight: 'bold',
        fill: '#0d2747',
        fontFamily: 'Georgia',
      ...
      const normTitle = titleTextStr.replace(/<[^>]*>/g, '').trim();
      if (rawBodyText.includes(normTitle)) {
        // Simple strip if body matches perfectly or starts with it
        // Note: Real world data is messy, this is a heuristic
        if (rawBodyText.trim().indexOf(normTitle) === 0) {
          rawBodyText = rawBodyText.substring(normTitle.length);
        }
      }
      rawBodyText = rawBodyText.trim().replace(/^[\s\-:]+/, '');
      const bodyTextStr = rawBodyText;

      let textBeside = null;
      let textBelow = null;
      let bodyObj = null;

      // Calculate heights - create temporary text objects to measure actual height
      let calculatedTitleHeight = titleObj.height;
      if (!calculatedTitleHeight) calculatedTitleHeight = 25; // fallback

      let requiredHeight = 0;
      const titleHeightPlusPadding = calculatedTitleHeight + 10;

      if (hasImages) {
        // Calculate available space beside image
        let heightBeside = Math.max(0, imageHeight - titleHeightPlusPadding);

        if (heightBeside < 40) {
          // Title is too tall, push all text below
          textBelow = new fabric.Textbox(bodyTextStr, {
            width: contentWidth - (cardPadding * 2),
            fontSize: 12,
            fill: '#495057',
            fontFamily: 'Arial',
            lineHeight: 1.4, // Tighter line height
            textAlign: 'left', // Left align to avoid word gaps
            splitByGrapheme: true
          });
        } else {
          // Try to fit text beside - use a more accurate approach
          // Create a temporary textbox to measure how much text fits
          const tempTextBox = new fabric.Textbox(bodyTextStr, {
            width: contentWidth - imageWidth - (cardPadding * 3),
            fontSize: 14,
            fill: '#495057',
            fontFamily: 'Arial',
            lineHeight: 1.4, // Tighter line height
          ...
          // Calculate how many lines fit beside the image
          const maxLinesBeside = Math.floor(heightBeside / (14 * 1.5)); // fontSize * lineHeight

          if (maxLinesBeside >= 1) {
            // Find the character position that would fit in the available lines
            const charsPerLine = Math.floor((contentWidth - imageWidth - (cardPadding * 3)) / 7); // rough estimate for 14px font
            const maxCharsBeside = charsPerLine * maxLinesBeside;

            let splitIdx = Math.min(maxCharsBeside, bodyTextStr.length);
            if (bodyTextStr.length > maxCharsBeside) {
              const lastSpace = bodyTextStr.lastIndexOf(' ', maxCharsBeside);
              if (lastSpace > 0) splitIdx = lastSpace;
            }

            const textPart1 = bodyTextStr.substring(0, splitIdx).trim();
            const textPart2 = bodyTextStr.substring(splitIdx).trim();

            if (textPart1) {
              textBeside = new fabric.Textbox(textPart1, {
                width: contentWidth - imageWidth - (cardPadding * 3),
                fontSize: 14,
                fill: '#495057',
                fontFamily: 'Arial',
            ...
            if (textPart2) {
              textBelow = new fabric.Textbox(textPart2, {
                width: contentWidth - (cardPadding * 2),
                fontSize: 14,
                fill: '#495057',
                fontFamily: 'Arial',
        ...
        const imageSideH = imageHeight + (cardPadding * 2);
        const textSideH = (cardPadding) + calculatedTitleHeight + 10 + (textBeside ? textBeside.height + 10 : 0) + (textBelow ? textBelow.height + 10 : 0) + cardPadding;

        requiredHeight = Math.max(imageSideH, textSideH);

      } else {
        // No image
        bodyObj = new fabric.Textbox(bodyTextStr, {
          width: contentWidth - (cardPadding * 2),
          fontSize: 14,
          fill: '#495057',
          fontFamily: 'Arial',
        ...
        requiredHeight = (cardPadding) + calculatedTitleHeight + 10 + bodyObj.height + (cardPadding);
      }

      requiredHeight = Math.max(requiredHeight, 100);

      // Check for Page Overflow
      if (y + requiredHeight > 1050) {
        addNewPage();
        canvas = window.modernEditor.pages[window.modernEditor.pages.length - 1];
        y = 60;
      }
      ...
      const articleStartY = y;

      // 1. Article Background
      const articleBg = new fabric.Rect({
        left: leftMargin - 10,
        top: articleStartY,
        width: contentWidth + 20,
        height: requiredHeight,
        fill: isEven ? '#ffffff' : '#f8f9fa',
        stroke: '#e9ecef',
        strokeWidth: 1,
        rx: 12,
        ry: 12,
      ...
      canvas.add(articleBg);

      // 2. Accent Bar
      const accentBar = new fabric.Rect({
        left: imageOnLeft ? leftMargin + contentWidth : leftMargin - 10,
        top: articleStartY,
        width: 6,
        height: requiredHeight,
        fill: accentColor,
      ...
      // 3. Content Placement
      let currentContentY = articleStartY + cardPadding;

      // Place Title above everything
      titleObj.set({
        left: leftMargin + cardPadding,
        top: currentContentY,
        width: contentWidth - (cardPadding * 2), // Fit within card width
        textAlign: 'left'
      });
      if (titleObj.setSelectionStyles) titleObj.setSelectionStyles({ fontWeight: 'bold' }, 0, titleTextStr.length);
      canvas.add(titleObj);

      currentContentY += calculatedTitleHeight + 10;

      if (hasImages) {
        const imageX = imageOnLeft ? (leftMargin + cardPadding) : (leftMargin + contentWidth - imageWidth - cardPadding);
        const textX = imageOnLeft ? (imageX + imageWidth + cardPadding) : (leftMargin + cardPadding);

        // Place Image Frame with Placeholder
        const imagesToShow = imageUrls.slice(0, 1);
        for (let i = 0; i < imagesToShow.length; i++) {
          const imageUrl = imagesToShow[i];

          // Create image placeholder/frame
          const imageCard = new fabric.Rect({
            left: imageX,
            top: currentContentY,
            width: imageWidth,
            height: imageHeight,
          ...
          canvas.add(imageCard);

          // Async Image Load - place image within the placeholder bounds
          try {
            await new Promise((resolve) => {
              let fullImageUrl = imageUrl;

              if (typeof window.getImageUrl === 'function') {
                fullImageUrl = window.getImageUrl(imageUrl);
              } else if (!imageUrl.startsWith('http://') && !imageUrl.startsWith('https://')) {
                // FORCE BACKEND URL for local dev environment
                ...
                if (imageUrl.startsWith('/uploads/')) {
                  ...
                  // Assume it's just the filename, prepend /uploads/
                  fullImageUrl = `${backendUrl}/uploads/${imageUrl}`;
                }
              }

              console.log('📸 Loading image:', imageUrl, '→', fullImageUrl);

              fabric.Image.fromURL(fullImageUrl, (img) => {
                if (img && img.width && img.height) {
                  console.log('✅ Image loaded successfully:', img.width, 'x', img.height);

                  // Scale image to fill the placeholder bounds completely
                  ...
                  img.scale(scale);
                  img.set({
                    left: imageX + (imageWidth - img.width * scale) / 2, // Center horizontally
                    top: currentContentY + (imageHeight - img.height * scale) / 2, // Center vertically
                    selectable: false, // Image itself not selectable, only the frame
                    hasControls: false,
                    hasBorders: false,
                    evented: false, // Disable events on image, handle through frame
                    opacity: 1, // Ensure image is fully visible
                    clipPath: new fabric.Rect({ // Clip to placeholder bounds
                      left: imageX,
                      top: currentContentY,
                      width: imageWidth,
                      height: imageHeight,
                  ...
                  canvas.add(img);
                  // Bring image to front so it's visible within the frame
                  img.bringToFront();
                  canvas.renderAll();
                } else {
                  console.error('❌ Image failed to load or has no dimensions:', fullImageUrl);
                  // Add placeholder icon/text inside the frame
                  const placeholderText = new fabric.IText('📷', {
                    left: imageX + imageWidth / 2,
                    top: currentContentY + imageHeight / 2,
                    fontSize: Math.min(imageWidth, imageHeight) * 0.3,
                    fill: '#6c757d',
                    originX: 'center',
                    originY: 'center',
                    selectable: false,
                  ...
                  placeholderText.moveTo(imageCard);
                  canvas.renderAll();
                }
                resolve();
            ...
            const errorText = new fabric.IText('❌', {
              left: imageX + imageWidth / 2,
              top: currentContentY + imageHeight / 2,
              fontSize: Math.min(imageWidth, imageHeight) * 0.3,
              fill: '#dc3545',
            ...
            errorText.moveTo(imageCard);
            canvas.renderAll();
          }
        }

        let textCurrentY = currentContentY;
        ...
        if (textBeside) {
          ...
          canvas.add(textBeside);
          textCurrentY += textBeside.height + 4; // Tighter spacing
        }

        // Place Below Text
        if (textBelow) {
          const imageBottom = currentContentY + imageHeight;
          const textSideBottom = textCurrentY;
          const startBelowY = Math.max(imageBottom, textSideBottom) + 6; // Tighter spacing

          textBelow.set({
            left: leftMargin + cardPadding,
            top: startBelowY,
            width: contentWidth - (cardPadding * 2),
            textAlign: 'justify' // Below text justified for better readability
          });
          canvas.add(textBelow);

          // Update requiredHeight to include the below text
          const textBelowBottom = startBelowY + textBelow.height;
          requiredHeight = Math.max(requiredHeight, textBelowBottom - articleStartY + cardPadding);
        }

      } else {
        // No Image - use justify for body
        const textX = leftMargin + cardPadding;
        ...
        // Update requiredHeight for no-image case
        const bodyBottom = currentContentY + bodyObj.height;
        requiredHeight = Math.max(requiredHeight, bodyBottom - articleStartY + cardPadding);
      }

      // Ensure minimum height and add some buffer
      requiredHeight = Math.max(requiredHeight, 100) + 20; // Add 20px buffer

      // Update article background height
      articleBg.set({ height: requiredHeight });
      accentBar.set({ height: requiredHeight });

      // Update y
      y += requiredHeight + 10; // Even tighter spacing

      // Decorative separator
      if (itemIndex < categoryItems.length - 1) {
        const separator = new fabric.Line([leftMargin, y, leftMargin + contentWidth, y], {
          stroke: '#e9ecef',
          strokeWidth: 1,
          strokeDashArray: isEven ? [5, 5] : [10, 5],
          selectable: true
        });
        canvas.add(separator);
        y += 12; // Tighter spacing after separator
      }

      // Progress
      processedItems++;
      const progress = 20 + (processedItems / totalItems) * 70;
...
function refreshPropertyPanelValues(canvas, explicitObj) {
  ...
  // If there is a selection/object, show panel; otherwise hide
  if (!obj) {
...
async function setupModernEventsAsync() {
  try {
    ...
    console.error('Error in setupModernEventsAsync:', error);
...
function attachPropertyPanelEvents(canvas) {
  if (canvas._propertyEventsAttached) return;
...
async function setupZoomControls() {
  ...
  function updateZoom(value) {
    ...
    // Use Fabric.js native zoom with crisp rendering for sharp text
    if (window.modernEditor && window.modernEditor.pages) {
      window.modernEditor.pages.forEach(canvas => {
        ...
        // Set text rendering to crisp
        context.textBaseline = 'top';
        context.fontSmooth = 'never';
        context.webkitFontSmoothing = 'none';
        context.mozOsxFontSmoothing = 'grayscale';
        ...
        // Force re-render all objects with crisp settings
        canvas.getObjects().forEach(obj => {
          if (obj.type === 'i-text' || obj.type === 'text' || obj.type === 'textbox') {
            // Ensure text objects use crisp rendering
...
async function setupAllPanels() {
  ...
  if (window.NewsletterEditorEnhancements && window.NewsletterEditorEnhancements.DrawingTool) {
    ...
    console.warn('DrawingTool not available, skipping drawing tools setup');
  }
  ...
  if (drawBtn && drawingPanel && window.modernEditor.drawingTool) {
    ...
    drawBtn.onclick = (e) => {
      ...
      if (!isVisible) {
        ...
        window.modernEditor.drawingTool.canvas = window.modernEditor.pages[window.modernEditor.currentPageIndex];
        window.modernEditor.drawingTool.enable();
        updateDrawingUI();
      } else {
        window.modernEditor.drawingTool.disable();
      }
    };
  }
  ...
  if (window.NewsletterEditorEnhancements && window.NewsletterEditorEnhancements.GridSystem) {
    ...
    console.warn('GridSystem not available, skipping grid system setup');
  }
  ...
  // Grid toggle
  if (gridToggle && window.modernEditor.gridSystem) {
    gridToggle.onchange = (e) => {
      if (e.target.checked) {
        window.modernEditor.gridSystem.show();
      } else {
        window.modernEditor.gridSystem.hide();
      }
      window.modernEditor.gridSystem.enabled = e.target.checked;
    };
  }

  // Snap toggle
  if (snapToggle && window.modernEditor.gridSystem) {
    snapToggle.onchange = (e) => {
      if (e.target.checked) {
        window.modernEditor.gridSystem.enableSnap();
      } else {
        window.modernEditor.gridSystem.disableSnap();
      }
      window.modernEditor.gridSystem.snapEnabled = e.target.checked;
    };
  }
  ...
  document.querySelectorAll('.spacing-btn').forEach(btn => {
    btn.onclick = () => {
      if (!window.modernEditor.gridSystem) return;
      const spacing = parseInt(btn.getAttribute('data-spacing'), 10);
      window.modernEditor.gridSystem.spacing = spacing;
      ...
      if (window.modernEditor.gridSystem.enabled) {
        window.modernEditor.gridSystem.show();
      }
    };
  });
  ...
  // Add guides
  if (addVerticalGuide && window.modernEditor.gridSystem) {
    addVerticalGuide.onclick = () => {
      const centerX = window.modernEditor.canvas.width / 2;
      window.modernEditor.gridSystem.addGuide('vertical', centerX);
    };
  }

  if (addHorizontalGuide && window.modernEditor.gridSystem) {
    addHorizontalGuide.onclick = () => {
      const centerY = window.modernEditor.canvas.height / 2;
      window.modernEditor.gridSystem.addGuide('horizontal', centerY);
    };
  }

  // Clear guides
  if (clearGuides && window.modernEditor.gridSystem) {
    clearGuides.onclick = () => {
      if (confirm('Clear all guide lines?')) {
        window.modernEditor.gridSystem.clearGuides();
      }
    };
  }


  if (drawingCloseBtn && window.modernEditor.drawingTool) {
  ...
  if (brushToolBtn && window.modernEditor.drawingTool) {
    brushToolBtn.onclick = () => {
      window.modernEditor.drawingTool.disableEraser();
      updateDrawingUI();
    };
  }

  if (eraserToolBtn && window.modernEditor.drawingTool) {
    eraserToolBtn.onclick = () => {
      window.modernEditor.drawingTool.enableEraser();
      updateDrawingUI();
    };
  }

  if (brushSizeSlider && window.modernEditor.drawingTool) {
    brushSizeSlider.oninput = (e) => {
      const size = parseInt(e.target.value, 10);
      if (brushSizeValue) brushSizeValue.textContent = size;
      window.modernEditor.drawingTool.setBrushWidth(size);
    };
  }

  if (drawColorPicker && window.modernEditor.drawingTool) {
    drawColorPicker.oninput = (e) => {
      window.modernEditor.drawingTool.setBrushColor(e.target.value);
      window.modernEditor.drawingTool.disableEraser();
      updateDrawingUI();
    };
  }
  ...
  // Color swatches
  if (window.modernEditor.drawingTool) {
    document.querySelectorAll('.color-swatch[data-draw-color]').forEach(swatch => {
      swatch.onclick = () => {
        const color = swatch.getAttribute('data-draw-color');
        window.modernEditor.drawingTool.setBrushColor(color);
        if (drawColorPicker) drawColorPicker.value = color;
        window.modernEditor.drawingTool.disableEraser(); // Switch back to brush when color selected
        updateDrawingUI();
      };
    });
  }

  if (clearDrawingsBtn && window.modernEditor.drawingTool) {
    clearDrawingsBtn.onclick = () => {
      if (confirm('Are you sure you want to clear all drawings?')) {
        window.modernEditor.drawingTool.clearDrawings();
      }
    };
  }
  ...
  if (bringToFrontBtn) {
    bringToFrontBtn.onclick = () => {
      ...
      if (obj) {
        ...
        window.modernEditor.canvas.renderAll();
        showTemporaryMessage('Element brought to front');
      } else {
        showTemporaryMessage('Please select an element first');
      }
    };
  }
  ...
  if (sendToBackBtn) {
    sendToBackBtn.onclick = () => {
      ...
      if (obj) {
        ...
        window.modernEditor.canvas.renderAll();
        showTemporaryMessage('Element sent to back');
      } else {
        showTemporaryMessage('Please select an element first');
      }
    };
  ...
  if (bringForwardBtn) {
    bringForwardBtn.onclick = () => {
      ...
      if (obj) {
        ...
        window.modernEditor.canvas.renderAll();
        showTemporaryMessage('Element brought forward');
      } else {
        showTemporaryMessage('Please select an element first');
      }
    };
  ...
  if (sendBackwardBtn) {
    sendBackwardBtn.onclick = () => {
      ...
      if (obj) {
        ...
        window.modernEditor.canvas.renderAll();
        showTemporaryMessage('Element sent backward');
      } else {
        showTemporaryMessage('Please select an element first');
      }
    };
...
function populateModernSidebar(items) {
  ...
  // Define categories for General Newsletter (sidebar)
  const categories = [
    { id: 'major-events', name: 'Major Events' },
    { id: 'alumni-stories', name: 'Alumni Stories' },
    { id: 'ccc-events', name: 'CCC Events' },
...
function addItemToCanvas(item) {
  ...
  // Helper function to create text with italic support and balanced typography (local copy for this function)
  function createTextWithItalics(text, options) {
    // First, balance the text for better typography
    const balancedText = balanceTextContent(text, options.width || 400, options.fontSize || 16, options.fontFamily || 'Arial');
...
async function downloadHighQualityPDF() {
  try {
    ...
    showTemporaryMessage('Applying text balancing and preparing PDF... Please wait.');
    ...
    // Check if there are any images that might still be loading
    let hasImages = false;
    ...
    // If there are images, wait longer to ensure they're loaded
    if (hasImages) {
      console.log('Images detected, waiting for loading...');
    ...
    const filename = 'general-newsletter-' + new Date().toISOString().split('T')[0] + '.pdf';
    pdf.save(filename);
...
function downloadWordDoc() {
  try {
    ...
    let htmlContent = `
      ...
      <head>
        <meta charset="utf-8">
        <title>General Newsletter</title>
        <style>
    ...
    pages.forEach((pageCanvas, pageIndex) => {
      ...
      sortedObjects.forEach(obj => {
        if (obj.type === 'i-text' || obj.type === 'text' || obj.type === 'textbox') {
          ...
          // We need to extract text and images from groups
          obj.getObjects().forEach(innerObj => {
            if (innerObj.type === 'i-text' || innerObj.type === 'text' || innerObj.type === 'textbox') {
              ...
              const formattedText = text.replace(/\n/g, '<br>');
              htmlContent += `<p style="${styles.join('; ')}">${formattedText}</p>`;
            } else if (innerObj.type === 'image') {
              // Handle images within groups
              try {
                const imgData = innerObj.toDataURL({
                  format: 'png',
                  quality: 1,
                  multiplier: 1,
                  enableRetinaScaling: false
                ...
                const scaledWidth = Math.round(innerObj.width * innerObj.scaleX);
                const scaledHeight = Math.round(innerObj.height * innerObj.scaleY);
                const wordWidth = Math.min(scaledWidth, 600);
                const wordHeight = Math.round((wordWidth / scaledWidth) * scaledHeight);
                ...
                console.error('❌ Could not export image from group:', e);
              }
            }
          });
        }
    ...
    link.href = url;
    link.download = 'general-newsletter-' + new Date().toISOString().split('T')[0] + '.doc';
...
function getTitleForItem(item) {
  ...
  const titles = {
    'majorevents': item.eventTitle || item.title || firstLine || 'Major Event',
    'alumnistories': item.storyTitle || item.title || firstLine || 'Alumni Story',
    'cccevents': item.eventTitle || item.title || firstLine || 'CCC Event',
    'clubactivities': item.activityTitle || item.title || firstLine || 'Club Activity',
    'degreereview': item.reviewTitle || item.title || firstLine || 'Degree Review',
    'deptactivities': item.activityTitle || item.title || firstLine || 'Department Activity',
    'library': item.bookTitle || item.title || firstLine || 'Library',
    'map': item.locationName || item.title || firstLine || 'Map',
    'media': item.caption || item.title || firstLine || 'Media',
    'memberships': item.membershipTitle || item.title || firstLine || 'Membership',
    'others': item.title || firstLine || 'Other',
    'recreation': item.activityTitle || item.title || firstLine || 'Recreation',
    'research': item.researchTitle || item.title || firstLine || 'Research',
    'researcharticle': item.articleTitle || item.title || firstLine || 'Research Article',
    'scholarships': item.scholarshipName || item.title || firstLine || 'Scholarship',
    'seminars': item.seminarTitle || item.title || firstLine || 'Seminar',
    'trainingprogram': item.programTitle || item.title || firstLine || 'Training Program',
...
// Duplicate function removed - using the main one defined earlier in the file
