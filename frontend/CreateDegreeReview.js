...
document.addEventListener("DOMContentLoaded", () => {
  const createDegreeReviewForm = document.getElementById("createDegreeReviewForm");
  ...
  async function getFormData() {
    ...
    const formData = {
      title: document.getElementById("degreeReviewTitle")?.value.trim() || "",
  ...
  function loadDraftData() {
    ...
    if (isEditMode && savedDraft) {
      try {
        ...
        if (data.title)
          document.getElementById("degreeReviewTitle").value = data.title;
  ...
  function populateFormFromData(data = {}) {
    if (data.title) document.getElementById("degreeReviewTitle").value = data.title;
  ...
  if (typeof setupSubmissionView === "function") {
    const isSubmissionView = setupSubmissionView({
      category: "degreeReview",
      onDataReady: populateFormFromData,
      formElement: createDegreeReviewForm,
      extraDisableElements: [draftBtn],
    });

    if (isSubmissionView) {
      console.log("📖 Viewing Degree Review submission in read-only mode.");
    }
  }

  // Handle form submission
  if (createDegreeReviewForm) {
    createDegreeReviewForm.addEventListener("submit", async function (e) {
      e.preventDefault();
      ...
      try {
        ...
        Toast.success("Degree Review submitted for review!");
        ...
        createDegreeReviewForm.reset();
        
        // Delay redirect to allow toast to be visible
        setTimeout(() => {
          window.location.href = "degreeReview.html";
        }, 1500);
      } catch (err) {
  ...
  if (draftBtn) {
    draftBtn.addEventListener("click", async function (e) {
      ...
      if (createDegreeReviewForm) {
        createDegreeReviewForm.noValidate = true;
      }
