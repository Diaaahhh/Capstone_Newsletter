...
document.addEventListener("DOMContentLoaded", () => {
  const createMembershipsForm = document.getElementById("createMembershipsForm");
  ...
  async function getFormData() {
    ...
    const formData = {
      title: document.getElementById("membershipsTitle")?.value.trim() || "",
  ...
  function loadDraftData() {
    ...
    if (isEditMode && savedDraft) {
      try {
        ...
        if (data.title)
          document.getElementById("membershipsTitle").value = data.title;
  ...
  function populateFormFromData(data = {}) {
    if (data.title) document.getElementById("membershipsTitle").value = data.title;
  ...
  if (typeof setupSubmissionView === "function") {
    const isSubmissionView = setupSubmissionView({
      ...
      formElement: createMembershipsForm,
      extraDisableElements: [draftBtn],
    });

    if (isSubmissionView) {
      console.log("📖 Viewing Memberships submission in read-only mode.");
    }
  }

  // Handle form submission
  if (createMembershipsForm) {
    createMembershipsForm.addEventListener("submit", async function (e) {
      e.preventDefault();
      ...
      try {
        ...
        Toast.success("Memberships submitted for review!");
  ...
  if (draftBtn) {
    draftBtn.addEventListener("click", async function (e) {
      ...
      if (createMembershipsForm) {
        createMembershipsForm.noValidate = true;
      }
