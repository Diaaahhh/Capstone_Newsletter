...
document.addEventListener("DOMContentLoaded", () => {
  const createTrainingProgramForm = document.getElementById("createTrainingProgramForm");
  ...
  async function getFormData() {
    ...
    const formData = {
      title: document.getElementById("trainingProgramTitle")?.value.trim() || "",
  ...
  function loadDraftData() {
    ...
    if (isEditMode && savedDraft) {
      try {
        ...
        if (data.title)
          document.getElementById("trainingProgramTitle").value = data.title;
  ...
  function populateFormFromData(data = {}) {
    if (data.title) document.getElementById("trainingProgramTitle").value = data.title;
  ...
  if (typeof setupSubmissionView === "function") {
    const isSubmissionView = setupSubmissionView({
      ...
      formElement: createTrainingProgramForm,
      extraDisableElements: [draftBtn],
    });

    if (isSubmissionView) {
      console.log("📖 Viewing Training Program submission in read-only mode.");
    }
  }

  // Handle form submission
  if (createTrainingProgramForm) {
    createTrainingProgramForm.addEventListener("submit", async function (e) {
      e.preventDefault();
      ...
      try {
        ...
        setTimeout(() => {
          window.location.href = "trainingProgram.html";
        }, 1500);
      } catch (err) {
  ...
  if (draftBtn) {
    draftBtn.addEventListener("click", async function (e) {
      ...
      if (createTrainingProgramForm) {
        createTrainingProgramForm.noValidate = true;
      }
