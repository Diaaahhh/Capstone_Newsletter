...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof setupSubmissionView === "function") {
    const isSubmissionView = setupSubmissionView({
      ...
      formElement: createMapForm,
      extraDisableElements: [draftBtn],
    });

    if (isSubmissionView) {
      console.log("📖 Viewing Map submission in read-only mode.");
    }
  }

  // Handle form submission
  if (createMapForm) {
    createMapForm.addEventListener("submit", async function (e) {
      e.preventDefault();
