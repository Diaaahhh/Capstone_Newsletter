...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function getFormData() {
    ...
    const formData = {
      title: document.getElementById("recreationTitle")?.value.trim() || "",
  ...
  function loadDraftData() {
    ...
    if (isEditMode && savedDraft) {
      try {
        ...
        if (data.title)
          document.getElementById("recreationTitle").value = data.title;
  ...
  function populateFormFromData(data = {}) {
    if (data.title) document.getElementById("recreationTitle").value = data.title;
  ...
  if (typeof setupSubmissionView === "function") {
    const isSubmissionView = setupSubmissionView({
      ...
      formElement: createRecreationForm,
      extraDisableElements: [draftBtn],
    });

    if (isSubmissionView) {
      console.log("📖 Viewing Recreation submission in read-only mode.");
    }
  }

  // Handle form submission
  if (createRecreationForm) {
    createRecreationForm.addEventListener("submit", async function (e) {
      e.preventDefault();
      ...
      try {
        ...
        Toast.success("Recreation submitted for review!");
  ...
  if (draftBtn) {
    draftBtn.addEventListener("click", async function (e) {
      ...
      if (createRecreationForm) {
        createRecreationForm.noValidate = true;
      }
