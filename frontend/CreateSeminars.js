...
document.addEventListener("DOMContentLoaded", () => {
  const createSeminarsForm = document.getElementById("createSeminarsForm");
  ...
  async function getFormData() {
    ...
    const formData = {
      title: document.getElementById("seminarsTitle")?.value.trim() || "",
  ...
  function loadDraftData() {
    ...
    if (isEditMode && savedDraft) {
      try {
        ...
        if (data.title)
          document.getElementById("seminarsTitle").value = data.title;
  ...
  function populateFormFromData(data = {}) {
    if (data.title) document.getElementById("seminarsTitle").value = data.title;
  ...
  if (typeof setupSubmissionView === "function") {
    const isSubmissionView = setupSubmissionView({
      ...
      formElement: createSeminarsForm,
      extraDisableElements: [draftBtn],
    });

    if (isSubmissionView) {
      console.log("📖 Viewing Seminars submission in read-only mode.");
    }
  }

  // Handle form submission
  if (createSeminarsForm) {
    createSeminarsForm.addEventListener("submit", async function (e) {
      e.preventDefault();
      ...
      try {
        ...
        Toast.success("Seminars submitted for review!");
  ...
  if (draftBtn) {
    draftBtn.addEventListener("click", async function (e) {
      ...
      if (createSeminarsForm) {
        createSeminarsForm.noValidate = true;
      }
